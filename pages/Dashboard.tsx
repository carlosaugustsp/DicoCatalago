...
import { Plus, Trash2, Edit2, Search, Package, X, User as UserIcon, Lock, LayoutDashboard, ChevronRight, ShoppingBag, Mail, Phone, Upload, Image as ImageIcon, FileSpreadsheet, Minus, RefreshCcw, Save, PlusCircle, Key, Lightbulb } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ user, refreshTrigger = 0 }) => {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Falha ao salvar produto: ${err.message || "Erro desconhecido"}`);
    }
  };
  ...
  const renderCRMBoard = () => (
    <div className="flex gap-4 overflow-x-auto pb-6 items-start custom-scrollbar h-[calc(100vh-260px)]">
      {Object.values(OrderStatus).map(status => (
        <div key={status} className="w-[280px] flex-shrink-0 bg-white rounded-2xl flex flex-col max-h-full border shadow-sm overflow-hidden">
          <div className={`p-4 ${getStatusColor(status)} text-white flex justify-between items-center sticky top-0 z-10`}>
          ...
          <div className="p-3 space-y-3 overflow-y-auto flex-grow bg-slate-50/50 custom-scrollbar">
            {orders.filter(o => o.status === status).map(order => (
              <div key={order.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 cursor-pointer hover:border-blue-500 transition-all group" onClick={() => setSelectedOrder(order)}>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                   <div className="flex items-center text-slate-500 text-[10px] font-black">
                      <ShoppingBag className="h-3 w-3 mr-1 text-slate-400"/> {order.items.reduce((a, i) => a + i.quantity, 0)} ITENS
                   </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b no-print overflow-x-auto">
            ...
            {activeTab === 'products' && (
              <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
                <div className="p-5 bg-slate-50 border-b flex justify-between items-center gap-4">
                  ...
                  <Button size="sm" className="font-black uppercase text-[10px]" onClick={() => { setEditingProduct({ colors: [], amperage: '' }); setShowProductModal(true); }}>
                        ...
                        <tr key={p.id} className="hover:bg-slate-50">
                          <td className="px-6 py-4 flex items-center gap-3">
                            ...
                            <div>
                               ...
                               <p className="text-[9px] text-slate-400 uppercase">{p.code}</p>
                            </div>
                          </td>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 z-[200] no-print">
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-10 overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-8">
               <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">{editingProduct.id ? 'Editar Produto' : 'Cadastrar Novo Produto'}</h3>
            ...
            <form onSubmit={handleSaveProduct} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Referência</label>
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Subcategoria</label>
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-6 bg-slate-800 p-6 rounded-2xl border border-slate-700 border-dashed">
                   {editingProduct.imageUrl ? (
                     <div className="relative w-24 h-24 rounded-lg border bg-white p-1 overflow-hidden group">
                        <img src={editingProduct.imageUrl} className="w-full h-full object-contain" alt="Preview"/>
                        <button type="button" onClick={() => setEditingProduct({...editingProduct, imageUrl: ''})} className="absolute inset-0 bg-red-500/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                           <Trash2 className="h-5 w-5"/>
                        </button>
                     </div>
                   ) : (
                     <div className="w-24 h-24 rounded-lg border-2 border-dashed border-slate-600 flex items-center justify-center text-slate-500 bg-slate-900/50">
                        <ImageIcon className="h-8 w-8"/>
                     </div>
                   )}
                   <div className="flex-grow w-full">
                      <input type="file" id="prod-img-upload" accept="image/*" className="hidden" onChange={handleImageUpload} />
                      <label htmlFor="prod-img-upload" className="flex items-center justify-center gap-3 w-full bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-xl cursor-pointer transition-all text-[11px] font-black uppercase">
                         <Upload className="h-5 w-5"/> SELECIONAR FOTO
                      </label>
                   </div>
                </div>
              </div>
              <div className="md:col-span-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Detalhes / Especificações</label>
              ...
              <div className="md:col-span-2 pt-4">
                <Button type="submit" className="w-full h-14 font-black uppercase tracking-widest">SALVAR REGISTRO</Button>
              </div>
            </form>
          </div>
        </div>
