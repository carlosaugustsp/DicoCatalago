...
export const Catalog: React.FC<CatalogProps> = ({ addToCart }) => {
  ...
  const getNovaraProducts = () => {
    ...
    if (novaraStep === 1) {
      // Passo 1: Apenas Placas
      novaraItems = novaraItems.filter(p => isPlateProduct(p));
    } else {
      // Passo 2: Módulos por padrão, mas permite Placas se a categoria for selecionada
      if (novaraModuleCategory === 'Placas') {
          novaraItems = novaraItems.filter(p => isPlateProduct(p));
      } else if (novaraModuleCategory === 'all') {
          // Se "Todos", mostra apenas módulos para não poluir
          novaraItems = novaraItems.filter(p => !isPlateProduct(p));
      } else {
          // Categoria específica de módulo
  ...
  const novaraColorsAvailable = ['all', ...Array.from(new Set(allNovaraProducts.flatMap(p => p.colors || []))).sort()];
  
  // Inclui explicitamente 'Placas' nas categorias de módulos para o Passo 2
  const novaraModuleCategories = ['all', 'Placas', ...Array.from(new Set(
    allNovaraProducts
      .filter(p => !isPlateProduct(p))
      .map(p => p.category)
  )).sort()];
        ...
        <>
          ...
          {loading ? (
            ...
            <>
                ...
                {filteredProducts.slice(0, visibleCount).map(product => (
                  <div key={product.id} className="bg-white rounded-lg shadow hover:shadow-md transition-shadow duration-200 flex flex-col h-full border border-gray-100">
                    <div className="relative w-full pt-[100%] bg-gray-200 rounded-t-lg overflow-hidden">
                      <img src={product.imageUrl} alt={product.description} className="absolute top-0 left-0 w-full h-full object-cover" loading="lazy" />
                      <span className="absolute top-2 right-2 bg-slate-800 text-white text-[10px] md:text-xs px-1.5 py-0.5 rounded opacity-90">{product.code}</span>
                    </div>
                    
                    <div className="p-3 md:p-4 flex-grow flex flex-col">
                      <span className="text-[10px] md:text-xs font-semibold text-blue-600 uppercase tracking-wide truncate block">{product.line}</span>
                      <h3 className="text-sm md:text-lg font-medium text-gray-900 mb-1 line-clamp-2 leading-tight">{product.description}</h3>
                      <p className="text-xs md:text-sm text-gray-500 mb-3">Ref: {product.reference}</p>
                      
                      <div className="mt-auto pt-2 space-y-2">
                        {product.details && (
                          <Button variant="outline" className="w-full text-xs py-1 h-8" onClick={() => setSelectedProductForInfo(product)}>
                        ...
                        >
                          {addedIds.includes(product.id) ? <Check className="h-3 w-3 mr-1" /> : <Plus className="h-3 w-3 mr-1" />}
                          {addedIds.includes(product.id) ? "Adicionado" : "Adicionar"}
                        </Button>
                      </div>
                    </div>
                  </div>
              ...
              {visibleCount < filteredProducts.length && <div ref={observerTarget} className="py-8 text-center text-gray-500 text-sm">Carregando mais...</div>}
            </>
          )}
        </>
      )}
        ...
        <div className="flex flex-col md:flex-row gap-6 h-full">
          <div className="flex-1">
             <div className="bg-slate-800 text-white p-4 rounded-lg mb-4 flex items-center justify-between shadow-lg">
                <div>
                   ...
                   <p className="text-xs text-slate-300">
                     {novaraStep === 1 ? 'Selecione uma placa para iniciar ou compre avulso.' : 'Adicione módulos ou troque a placa atual.'}
                   </p>
                </div>
             ...
             <div className="bg-white p-3 rounded-lg border border-gray-200 mb-4 flex gap-2 flex-wrap items-center">
                <div className="relative flex-grow min-w-[200px]">
                   ...
                   <input
                      type="text"
                      className="w-full pl-8 pr-2 py-1.5 text-sm border rounded bg-gray-50"
                      placeholder="Buscar..."
                      value={novaraSearch}
                ...
                <select className="p-1.5 text-sm border rounded bg-gray-50" value={novaraColor} onChange={(e) => setNovaraColor(e.target.value)}>
                  <option value="all">Todas Cores</option>
                  {novaraColorsAvailable.filter(c => c !== 'all').map(c => <option key={c} value={c}>{c}</option>)}
                </select>

                <select className="p-1.5 text-sm border rounded bg-gray-50 min-w-[150px]" value={novaraModuleCategory} onChange={(e) => setNovaraModuleCategory(e.target.value)}>
                  <option value="all">{novaraStep === 2 ? 'Categorias / Placas' : 'Todas Placas'}</option>
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
               {getNovaraProducts().map(product => {
                 ...
                 return (
                   <div key={product.id} className={`bg-white border rounded-lg overflow-hidden transition-all ${selectedPlate?.id === product.id ? 'ring-2 ring-blue-500' : ''}`}>
                     <div className="relative pt-[100%] bg-gray-100">
                        <img src={product.imageUrl} className="absolute inset-0 w-full h-full object-cover" alt="" />
                        {isPlate && (
                          <span className="absolute top-2 left-2 bg-blue-600 text-white text-[9px] px-1.5 py-0.5 rounded uppercase font-bold">Placa</span>
                        )}
                     </div>
                     <div className="p-3">
                        <h4 className="font-medium text-sm text-gray-800 line-clamp-2 min-h-[40px]">{product.description}</h4>
                        <p className="text-[10px] text-gray-400 mt-1">{product.code}</p>
                        
                        {isPlate ? (
                           <div className="grid grid-cols-2 gap-2 mt-2">
                               <Button variant="outline" size="sm" className="text-[10px] px-1 py-1" onClick={() => handleAddToCart(product)}>
                                  <ShoppingCart className="h-3 w-3 mr-1"/> Avulso
                               </Button>
                               <Button size="sm" className={`text-[10px] px-1 py-1 ${selectedPlate?.id === product.id ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'} text-white`} onClick={() => { setSelectedPlate(product); setNovaraStep(2); }}>
                                  <Layers className="h-3 w-3 mr-1"/> {selectedPlate?.id === product.id ? 'No Kit' : 'Usar no Kit'}
                               </Button>
                           </div>
                        ) : (
                           <Button size="sm" className="w-full mt-2" onClick={() => addModuleToKit(product)}>
                              <Plus className="h-3 w-3 mr-1"/> Adicionar ao Kit
                           </Button>
                        )}
                     </div>
                   </div>
          ...
          <div className="w-full md:w-80 bg-white rounded-lg shadow-lg border border-gray-200 flex flex-col h-fit sticky top-24">
             <div className="p-4 bg-slate-900 text-white rounded-t-lg flex justify-between items-center">
                <h3 className="font-bold flex items-center text-sm"><Layers className="h-4 w-4 mr-2"/> Seu Conjunto</h3>
                {selectedPlate && <button onClick={() => { setSelectedPlate(null); setNovaraStep(1); }} className="text-[10px] text-blue-400 hover:underline">Reiniciar</button>}
             </div>
             
             <div className="p-4 flex-grow space-y-4">
                <div className="border-b pb-4">
                   <p className="text-[10px] font-bold text-gray-400 uppercase mb-2">Base Selecionada</p>
                   {selectedPlate ? (
                     <div className="flex items-center gap-3">
                        <img src={selectedPlate.imageUrl} className="w-10 h-10 rounded object-cover border" alt=""/>
                        <div className="flex-1 overflow-hidden">
                           <div className="text-xs font-bold text-gray-800 truncate">{selectedPlate.description}</div>
                           <div className="text-[9px] text-gray-500">{selectedPlate.code}</div>
                        </div>
                     </div>
                   ) : (
                     <div className="text-xs text-gray-400 italic bg-gray-50 p-4 rounded text-center border border-dashed">Selecione uma placa</div>
                   )}
                </div>

                <div>
                   <div className="flex justify-between items-center mb-2">
                      <p className="text-[10px] font-bold text-gray-400 uppercase">Módulos ({selectedModules.reduce((a,b) => a + b.qty, 0)})</p>
                   </div>
                   <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                      {selectedModules.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-gray-50 p-2 rounded border">
                           <div className="flex-1 min-w-0 mr-2"><div className="text-[10px] font-medium truncate">{item.product.description}</div></div>
                           <div className="flex items-center gap-2">
                             <span className="text-[10px] font-bold">x{item.qty}</span>
             ...
             <div className="p-4 border-t bg-gray-50 rounded-b-lg space-y-2">
                <Button className="w-full h-12 text-xs" disabled={!selectedPlate} onClick={addKitToCart}>Adicionar Conjunto ao Carrinho</Button>
                {novaraStep === 2 && <Button variant="outline" className="w-full h-10 text-[10px]" onClick={() => setNovaraStep(1)}>Ver Todas as Placas</Button>}
             </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-[70] backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-2xl max-w-lg w-full relative overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="font-bold text-lg text-gray-800 flex items-center"><FileText className="h-5 w-5 mr-2 text-blue-600"/> Detalhes</h3>
            ...
            <div className="p-6 overflow-y-auto">
               ...
               <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 text-sm text-blue-900 whitespace-pre-wrap">{selectedProductForInfo.details}</div>
            </div>
            <div className="p-4 border-t bg-gray-50 text-right"><Button onClick={() => setSelectedProductForInfo(null)}>Fechar</Button></div>
          </div>
        </div>
      )}
      ...
      {showHelp && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6 relative">
            <button onClick={() => setShowHelp(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><X className="h-6 w-6"/></button>
            <h3 className="text-xl font-bold mb-4">Como Comprar?</h3>
            <ol className="list-decimal pl-5 space-y-3 text-gray-700 text-sm">
              <li>Explore o catálogo geral ou use o <strong>Monte sua Novara</strong> para conjuntos personalizados.</li>
              <li>No construtor Novara, escolha uma placa e adicione módulos (tomadas, interruptores, etc).</li>
              <li>Você pode adicionar placas avulsas mesmo enquanto monta um conjunto selecionando a categoria "Placas".</li>
              <li>Vá ao carrinho, revise e preencha os dados da sua revenda para enviar ao representante.</li>
            </ol>
            <div className="mt-6 text-right"><Button onClick={() => setShowHelp(false)}>Entendi</Button></div>
          </div>
        </div>
      )}
    </div>
